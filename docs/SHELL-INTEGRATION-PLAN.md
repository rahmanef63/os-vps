...
| 4 | **Widgets** | full editable framework — 12-col grid canvas, gallery picker, per-space persistence, 13 S/M/L widget types | **editable stack** — picker (add/remove/reorder) + **per-widget right-click S/M/L resize**, **15 types**: cpu/mem/disk/net/uptime/clock/calendar/tasks/timer/notes/html/embed/quicklinks/theme/shell-picker, live VPS telemetry + CPU/net sparklines, persisted | ↔ near-parity | ✅ **Shipped** — widget *types* now ≥ shell (shell's `system` = os-vps's split cpu/mem/disk/net; `prayer`/`email` skipped as portfolio). Only edge left: free 12-col drag grid + per-space — deferred; vertical glance-stack fits the VPS cockpit |
| 5 | **Context menu / right-click** | portal-stacked, arrow-key nav, focus-restore, Fluent variant; surfaces: desktop, icons, **window title-bar**, dock, taskbar, widget | **registry-based** (`registerContextMenu`) + now **window title-bar menu** (Maximize/Minimize/Pin/Close), **View as {shell}** + **Full Screen** on desktop, per-widget S/M/L, dock, taskbar | ↔ parity (os-vps registry is more extensible) | ✅ **Shipped**. Remaining shell-only: desktop-icon menu (no desktop icons yet — deferred) |
